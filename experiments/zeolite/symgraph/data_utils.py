...
from symgraph.ks_frozen import load_or_compute_ks_features, KS_DIM
...
def get_genpos_generators(space_group_number):
    """
    Offline space-group generators from Hall notation.

    Replaces the live Bilbao GENPOS request, which can be blocked
    by the BCS Turnstile challenge on remote compute servers.
    """
    import gemmi

    number = int(space_group_number)

    sg = gemmi.get_spacegroup_reference_setting(number)
    if sg is None:
        raise RuntimeError(
            f"Gemmi has no reference setting for space group {number}"
        )
    ...
    # Hall symmetry generators.
    for op in generators.sym_ops:
        m = np.asarray(op.float_seitz(), dtype=float)
        mats.append(m)

    # Hall lattice-centring generators.
    # Gemmi translations use denominator 24.
    for tran in generators.cen_ops:
        t = np.asarray(tran, dtype=float) / 24.0
    ...
    if not mats:
        raise RuntimeError(
            f"No Hall generators obtained for space group {number}"
        )

    # Canonicalize translation and remove exact duplicates.
    unique = []
    seen = set()
    ...
    for m in mats:
        ...
        key = tuple(np.round(m, 12).ravel())

        if key not in seen:
            seen.add(key)
            unique.append(m)

    out = np.stack(unique, axis=0)

    print(
        f"[OFFLINE-GEN] SG={number} "
        f"Hall='{sg.hall}' generators={len(out)}"
    )
...
def get_edge_index_with_vec(x, x_o, len_zeo, ang_zeo, edge_type='zeo', radius=5.0):
    """Return edge_index, scalar distance, and periodic Cartesian edge vector."""
    if edge_type == 'radius':
        x_t = torch.tensor(x, dtype=torch.float)
        x_i = x_t
        x_j = (x_t[:, None] + OFFSETS[None]).reshape(-1, 3)
        j_indices = torch.arange(len(x_t)).reshape(-1, 1).repeat(1, 27).reshape(-1)
        h = torch.tensor(get_transform_matrix(*len_zeo, *ang_zeo), dtype=torch.float)
        diff_cart = frac_to_cart((x_i[:, None] - x_j), h)
        dist = diff_cart.norm(dim=-1)
        mask = (dist < radius) & (dist > 1e-5)
        i, j_img = torch.where(mask)
        j = j_indices[j_img]
        edge_index = torch.stack([i, j], dim=0)
        edge_attr = dist[mask]
        edge_vec = diff_cart[mask]
        return edge_index, edge_attr, edge_vec

    edge_index, edge_attr = get_edge_index(
        x, x_o, len_zeo, ang_zeo, edge_type=edge_type, radius=radius
    )
    i, j = edge_index
    x_np = np.asarray(x)
    diff = periodic_boundary(x_np[i.cpu().numpy()] - x_np[j.cpu().numpy()])
    h = torch.tensor(get_transform_matrix(*len_zeo, *ang_zeo), dtype=torch.float)
    edge_vec = frac_to_cart(torch.tensor(diff, dtype=torch.float), h)
    return edge_index, edge_attr, edge_vec


def create_graphs_zeo(zeo, pos, atoms, lat_params, wyck, equi, hoa, hoa_err, iso_params, edge_index=None, edge_attr=None, edge_vec=None, wyck_dict=None):
    '''
    ...
    # KSGL FROZEN FEATURE ATTACHMENT
    # This is topology-only and is computed once per zeolite framework, then reused
    # for every Al/Si configuration belonging to that framework.
    if os.environ.get("USE_KS_FEATURES", "0") == "1":
        ks_np = load_or_compute_ks_features(
            zeo=zeo, n=len(pos), edge_index=edge_index,
            data_root=PROJECT_ROOT / "Data_numpy",
            radius=int(os.environ.get("KS_RADIUS", "3")),
            local_cap=int(os.environ.get("KS_LOCAL_CAP", "512")),
            global_samples=int(os.environ.get("KS_GLOBAL_SAMPLES", "10000")),
            normalization=os.environ.get("KS_NORMALIZATION", "probability"),
            base_seed=int(os.environ.get("KS_SEED", "42")),
        )
        ks = torch.tensor(ks_np, dtype=torch.float)
    else:
        ks = torch.zeros((len(pos), KS_DIM), dtype=torch.float)
...
def get_wyck_dict(crystal, sga):
    ...
    # Compatibility bypass for the official GNN baseline.
    # Bilbao GENPOS is currently protected by Cloudflare Turnstile.
    # The official GNN has hyper_node_type='none' and hyper_edge_type='none',
    # therefore these symmetry generators are not used by the GNN forward pass.
    if os.environ.get("SYMGNN_OFFLINE_GNN", "0") == "1":
        ident = SymmOp(np.eye(4))
        return {w: [ident] for w in set(crystal.wyckoff_symbols)}
...
def create_graphs(zeo_codes='all', val_split=0.0, test_split=0.0, edge_type='zeo', radius=5.0):
    ...
    for zeo in zeo_codes:
        ...
        edge_index, edge_attr, edge_vec = get_edge_index_with_vec(pos, pos_o, lens, angs, edge_type, radius)
        ...
        new_graphs = create_graphs_zeo(zeo, pos, ats, [lens, angs], wyck, equi, hoa, hoa_err, iso_params, edge_index, edge_attr, edge_vec, wyck_dict)
