...
class GNN(nn.Module):
    ...
    def __init__(self,
        ...
        # KSGL FROZEN GATED RESIDUAL
        # Same equation as the frozen ZeoDiff KSGL: project KS, gate [h||z],
        # then h_tilde = h + beta * gate * z at EVERY message-passing layer.
        self.use_ks = bool(use_ks)
        if self.use_ks:
            self.ks_proj = nn.ModuleList([
                nn.Sequential(nn.Linear(int(ks_dim), hidden), nn.ELU(), nn.LayerNorm(hidden))
                for _ in range(num_layers)
            ])
            self.ks_gates = nn.ModuleList([nn.Linear(2 * hidden, hidden) for _ in range(num_layers)])
            for gate in self.ks_gates:
                nn.init.constant_(gate.bias, float(gate_bias))
            self.ks_beta = nn.Parameter(torch.full((num_layers,), float(beta_init)))
    ...
    def forward(self, data, pres=None, integrate=False):
        ...
        for layer in range(self.num_layers):
            ...
            # KSGL: enhance the current node state BEFORE official message passing.
            if self.use_ks:
                z = self.ks_proj[layer](data.ks)
                a = torch.sigmoid(self.ks_gates[layer](torch.cat([x, z], dim=-1)))
                x_layer = x + self.ks_beta[layer] * a * z
            else:
                x_layer = x

            layer_idx_edge = torch.zeros((edge_index.size(1),1), dtype=torch.float, device=x.device) + layer
            layer_idx_node = torch.zeros((x_layer.size(0),1), dtype=torch.float, device=x.device) + layer

            # calculate messages
            i,j = edge_index

            m_ij = torch.cat([x_layer[i], x_layer[j], edge_emb], dim=-1)
            ...
            if self.node_agg == 'mean':
                m_i = scatter_mean(m_ij, i, dim=0, dim_size=x_layer.size(0))
            elif self.node_agg == 'sum':
                m_i = scatter_add(m_ij, i, dim=0, dim_size=x_layer.size(0))
            else:
                raise NotImplementedError

            # update nodes
            h = torch.cat([x_layer, m_i], dim=-1)

            if self.hyper_node_type == 'wyckoff':
                h = self.node_update_1(h, layer_idx_node, wyck, mults)
            elif self.hyper_node_type == 'gens_set_film':
                h = self.node_update_1[layer](h, layer_idx_node, gens, gen_idx, torch.arange(x_layer.size(0), dtype=torch.long, device=x.device))
            elif self.hyper_node_type == 'none':
