...
from symgraph.model.benchmark_models import ALIGNNMultiTask, MatformerMultiTask
...
def run(cfg: DictConfig):
    ...
    # KSGL_3SEED_BEST400_PATCH
    # Keep dataset.seed fixed for the split; vary only training randomness.
    ...
    print(
        ...
        f"DATASET_SEED={cfg.dataset.seed}"
    )

    model_name = str(cfg.model.name).lower()
    if model_name == 'alignn':
        model = ALIGNNMultiTask(**cfg.model).to(cfg.device)
    elif model_name == 'matformer':
        model = MatformerMultiTask(**cfg.model).to(cfg.device)
    else:
    ...
    wandb.init(project=cfg.logging.wandb.project, entity=cfg.logging.wandb.entity, name=cfg.expname, config=cfg_dict, mode=cfg.logging.wandb.mode)
    ...
    for epoch in range(cfg.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(
                model.state_dict(),
                f'{PROJECT_ROOT}/models/{cfg.expname}/best.pth'
            )
            print(
                f"BEST_UPDATE "
    ...
    best_path = f'{PROJECT_ROOT}/models/{cfg.expname}/best.pth'
    ...
    true, pred, true_iso, pred_iso = test(
        model,
        testloader,
        cfg.device,
        mu,
        std,
        pres_points=cfg.isotherm.n_steps
    )
